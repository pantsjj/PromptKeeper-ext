...
// AI-Optimize Button
document.getElementById('ai-optimize-button').addEventListener('click', async function () {
    const promptTextArea = document.getElementById('prompt-text');
    const promptText = promptTextArea.value.trim();
    const contextText = promptText; // Using the prompt text as context
    ...
    try {
        // Check if window.ai and window.ai.rewriter are available
        if (!window.ai || !window.ai.rewriter) {
            alert('AI features are not available in this browser.');
            console.log('window.ai or window.ai.rewriter is not available.');
            aiProcessing = false;
            aiOptimizeButton.textContent = 'AI-Optimize';
            removeSpinner();
            return;
        ...
        // Create options based on rewrite option
        let options = {};
        if (rewriteOption !== 'retry') {
            if (rewriteOption === 'more-formal' || rewriteOption === 'more-casual') {
                options.tone = rewriteOption;
            } else if (rewriteOption === 'shorter' || rewriteOption === 'longer') {
                options.length = rewriteOption;
            }
        }

        // Create a new rewriter instance with options
        const rewriter = await window.ai.rewriter.create({ ...options, signal });

        // Use rewriteStreaming to get streaming output
        const stream = await rewriter.rewriteStreaming(promptText, { context: contextText, signal });
        let rewrittenPrompt = '';

        // Prepare to append to the existing text
        let originalPrompt = promptTextArea.value;

        for await (const chunk of stream) {
            rewrittenPrompt = chunk;

            // Optionally, append the chunk to the prompt text area
        ...
        // Append the final rewritten prompt to the text area
        promptTextArea.value = originalPrompt + '\n\n---\n\n' + rewrittenPrompt;
        ...
        if (error.name === 'AbortError') {
            ...
            alert('Error rewriting prompt. Please ensure the AI features are enabled and try again.');
        }
