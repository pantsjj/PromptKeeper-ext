...
- [ ] Refactor `popup.js` to import and use these services instead of direct API calls.
...
- [ ] Configure `manifest.json` to register the options page.
...
- [ ] Add an "Open Full Editor" button to `popup.html`.
...
**Acceptance Criteria**:
- [ ] Define CSS variables for colors (bg, text, primary, accent).
- [ ] Implement a toggle for Light/Dark mode.
- [ ] Apply basic "Card" styling to prompt list items.
